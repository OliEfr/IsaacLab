...
class TerrainGenerator:
    ...
    def _generate_random_terrains(self):
        ...
        for index in range(self.cfg.num_rows * self.cfg.num_cols):
            ...
            # generate terrain
            mesh, origin, terrain_params = self._get_terrain_mesh(difficulty, sub_terrains_cfgs[sub_index])
            # add to sub-terrains
    ...
    def _get_terrain_mesh(self, difficulty: float, cfg: SubTerrainBaseCfg) -> tuple[trimesh.Trimesh, np.ndarray]:
        ...
        meshes, origin, *rest = cfg.function(difficulty, cfg)
        # Optionally you can return additional parameters generated by the terrain function. This can be useful to keep track of parameters that were used for terrain generation.
        if len(rest) > 0:
            terrain_params = rest[0]
        else:
            terrain_params = None
        ...
        # return the generated mesh
        return mesh, origin, terrain_params
